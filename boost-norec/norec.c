...
// returns 1 on success, -1 on "cannot do it with HTM, please try STM"
// in case of validation abort, it does the longjmp to restart the STM
int TxCommitHTM (Thread *Self) {
    int tries = 4;
    while (tries) {
        Log* const rd = &Self->rdSet;
        AVPair* const EndOfList = rd->put;
        ...
        if (status == _XBEGIN_STARTED) {
            ...
            _xend();
            return 1;
        } else if ((status & _XABORT_EXPLICIT) && _XABORT_CODE(status) == 30) {
            TxAbort(Self);
            return 0;
        }
        tries--;
